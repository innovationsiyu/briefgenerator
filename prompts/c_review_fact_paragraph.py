...
- 第一，严格基于source_text，对fact_paragraph依次执行行动清单中的10项检查，将这些检查结果包裹在XML标签<feedback_on_fact_paragraph>中输出。
- 第二，根据这些检查结果，确定<corrections_required>应该为True还是False，包裹在XML标签<corrections_required>中输出。
...
9. fact_paragraph必须直接呈现新闻文章中的信息，避免使用“文章总结了”“文章认为”这样的表述来引出新闻文章中的信息，无需说明这些信息出自新闻文章。指出应删除的此类表述，或者输出“完全符合要求”。
...
- 如果10项检查结果都是“完全一致”或“完全正确”或“完全符合要求”，则<corrections_required>为False，否则为True。这对XML标签内部仅仅使用True或False，避免任何额外内容。
