...
json_format = {
  "1": {
    ...
    "新闻文章原文": "从新闻文章中完整摘抄与第1句中的事实或观点对应的原文，或者回答“没有找到对应的原文”",
    "有无曲解原意": "基于对应的原文，检查第1句中的事实或观点是否符合原意；如果没有找到对应的原文，分析第1句中的事实或观点是否正确总结了新闻文章的内容。输出思考过程，最后回答“没有曲解原意”，或者回答“应整体删除这一句”"
  },
  "n": {
    "检查的句子": "完整抄写第n句",
...
- 第一，严格基于source_text，按照序号依次检查每一个fact_sentence中有没有与前面的fact_sentences重复的信息，有没有不属于source_text中的新闻文章的事实或观点，以JSON格式呈现对每一句的检查结果，包裹在XML标签<feedback_on_fact_sentences>中输出。
- 第二，根据这些检查结果中有没有至少一个fact_sentence需要修改或删除，确定<corrections_required>应该为True还是False，包裹在XML标签<corrections_required>中输出。
...
- 请严格按照输出格式示例中的要求以JSON格式呈现对每一句的检查结果。
- 如果对全部n句的检查结果都是“没有重复信息”和“没有曲解原意”，则<corrections_required>为False，否则为True。这对XML标签内部仅仅使用True或False，避免任何额外内容。
